...
<div class="md:col-span-3 px-[10px] space-y-10 order-1 md:order-0 mt-10 md:mt-0">
    <form class="relative" action="{{route('homepage.search')}}">
        <input type="text" name="keyword" class="text-black w-full px-10px h-[45px] bg-[#f7f7f7] border text-sm focus:outline-none hover:outline-none outline-none" placeholder="{{trans('index.SearchPlaceholder')}}">
    ...
    <aside class="">
        ...
        <ul class="">
            ...
            <li class="relative py-[5px] flex space-x-[10px]">
                ...
                <a href="{{route('routerURL',['slug' => $item->slug])}}" class="hover:text-primary flex-1 lg:text-f14">{{$item->title}}</a>
            </li>
            @endforeach
        </ul>
    </aside>
    ...
    <div class="">
        ...
        <ul class="flex flex-wrap">
            ...
            <li>
                <a href="{{route('tagURL',['slug' => $item->slug])}}" class="border p-2 float-left mr-2 mb-2 hover:bg-primary hover:border-primary hover:text-white">{{$item->title}}</a>
            </li>
            @endforeach
        </ul>
    </div>
